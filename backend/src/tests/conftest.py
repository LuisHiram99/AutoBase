...
def client(async_session, _setup_workshop):
    ...
    # Create a mock limiter that doesn't actually limit anything
    mock_limiter = MagicMock()
    
    # Make the limit decorator return the function unchanged (no-op)
    def mock_limit_decorator(rate_string):
        def decorator(func):
    ...
    # Override the limiter in the app state
    original_limiter = getattr(app.state, 'limiter', None)
    app.state.limiter = mock_limiter
    
    # Import and override the limiter in the handler modules
    from handler import rate_limiter
    original_module_limiter = rate_limiter.limiter
    rate_limiter.limiter = mock_limiter
    
    with TestClient(app) as test_client:
        yield test_client
    
    # Restore original values after test to avoid side effects
    app.dependency_overrides.clear()
    if original_limiter is not None:
        app.state.limiter = original_limiter
    rate_limiter.limiter = original_module_limiter
